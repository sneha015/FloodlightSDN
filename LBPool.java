...
/* Random Algorithm Implimented by us using random number generator math.random(), this */
/* This API called in LoadBalancer.java class to pick the member form the pool.         */
/*######################################################################################*/
    public String pickMemberRandom(IPClient client) {
    	int random = (int )(Math.random() * members.size());
    if (members.size() > 0) {
    	previousMemberIndex = (previousMemberIndex + random) % members.size(); 
            return members.get(previousMemberIndex);
        } else {
            return null;
        }
